...
export const uploadImgAction = (file, token) => dispatch => {
	...
	return fetch(`${api_root}/upload/image`, {
		method: 'POST',
		headers: {
			Authorization: `Bearer ${token}`,
		},
...
export const getCountries = () => dispatch => {
	dispatch(fetchingAction());
	return FetchHelper.fetch(`${api_root}/common/getCountries`, {
	}).then(([data, status]) => {
		dispatch(fetchingFinishAction());
...
export const getCities = (param) => dispatch => {
	dispatch(fetchingAction());
	return FetchHelper.fetch(`${api_root}/common/getCities`, {
		method: 'POST',
