...
const VehicleModelPresenter = props => {
	const { list, onChangePage, onDropdownToggle, actionDropdownOpen, vehicleModelId, onDeleteVehicleModel } = props;
			...
			<Card>
				...
				<CardBody>
                    <Col sm={12} style={{ textAlign:'right', padding:10}}>
						<a href={`?#/configure/vehiclesModelInfo/add`}>
									...
									{list.map(item => (
										<tr key={item._id}>
											...
											<ButtonDropdown isOpen={(vehicleModelId === item._id) && actionDropdownOpen} toggle={onDropdownToggle}>
												...
												<DropdownMenu>
													<a href={`?#/configure/vehiclesModelInfo/${item._id}`}>
														<DropdownItem>Edit</DropdownItem>
													</a>
													<DropdownItem id={item._id} onClick={props.onDeleteVehicleModel}>Delete</DropdownItem>
												</DropdownMenu>
											</ButtonDropdown>
											</td>
										</tr>
...
export default VehicleModelPresenter;
