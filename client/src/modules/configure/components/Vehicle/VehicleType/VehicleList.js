...
import { getVehicleTypeListAction, deleteVehicleTypeAction } from '../../../action';
import VehicleListPresenter from './presenter';
...
class VehicleList extends Component {
	...
	onChangePage = page => {
		...
		dispatch(getVehicleTypeListAction(params, token)).then(
			([err, data]) =>
				!err &&
				this.setState({
	...
	async deleteAccount(e) {
		...
		dispatch(deleteVehicleTypeAction( payload, token)).then(
			setTimeout(()=>{
	...
	render() {
		const { dataList, params, filter, keyword, actionDropdownOpen, vehicleListId } = this.state;
		...
		return (
			<VehicleListPresenter
				...
				onDeleteVehicleType={this.deleteAccount}
			/>
		);
	}
}
